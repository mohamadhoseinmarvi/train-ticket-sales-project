...
w varchar(50):='RAILWAY MANAGEMENT SYSTEM';

begin

dbms_output.put_line('WELCOME TO ' || w);
end;
/
...
insert into Adminn values (1,'p','sss');
insert into Adminn values (2,'t','p');
...
s_id :=&s_id;
t_id:=&t_id;
select name into unm from Adminn where name=uname;
select pass into upa from Adminn where name=uname;

if upa=upass then
dbms_output.put_line('YOU ARE ALLOWED TO CONTINUE. WELCOME ' || uname);
delete from Passenger_ticket where seat_id=s_id and train_id=t_id;
else
dbms_output.put_line('YOU ARE NOT ALLOWED, '|| uname);
end if;
end;
/
